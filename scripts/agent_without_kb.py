...
def save_results_to_csv(results, output_dir, filename):
    ...
    try:
        ...
        df.to_csv("qwen.csv", index=False)
    print(f"Results saved to {full_path}")

# ------------------- LLM Diagnostic Agent -------------------
def run_diagnosis_agent(state: LLMState, llm, wait_time: int):
    """
    ...
    """
    prompt = DIAGNOSIS_PROMPT_TEMPLATE.format(patient_symptoms=state['prompt'])
...
def main():
    ...
    workflow.add_node("Diagnosis", lambda state: run_diagnosis_agent(state, llm, wait_time))
