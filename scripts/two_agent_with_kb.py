...
The patient has been diagnosed as {diagnosis}.
The reason of diagnosis is 
{reasoning}.
Please validate the diagnosis according to the reasoning and provide final diagnosis as 'Rheumatoid Arthritis' or 'Not Rheumatoid Arthritis'.\n\n
Do not output anything else.
"""
...
def save_results_to_csv(results, output_dir, filename):
    ...
    df = pd.DataFrame({
        ...
        "Validated Predictions": results[3],
    })
...
def run_validation_agent(state: LLMState, llm, wait_time: int):
    """
    Executes the reasoning agent to generate explanations for a given diagnosis.
    ...
    Returns:
        dict: A dictionary containing the following key:
            - 'reasons': The reasoning output generated by the LLM as a string.
    """
    prompt = VALIDATION_PROMPT_TEMPLATE.format(diagnosis=state['diagnosis'], reasoning=state['reasons'])
    response = llm.invoke(prompt)
    time.sleep(wait_time)
...
def main():
    ...
    parser.add_argument("--provider", type=str, required=True,
                    ...
                    help="LLM provider: GOOGLE, OPENAI, OLLAMA")

    # parser.add_argument("--provider", type=str, required=True, choices=["GOOGLE", "OPENAI", "OLLAMA"], help="LLM provider: GOOGLE or OPENAI")
    ...
    workflow.add_node("Validate", lambda state: run_validation_agent(state, llm, wait_time))
    ...
    for idx, patient_prompt in tqdm(enumerate(prompts), total=len(prompts), desc="Diagnosing Patients"):
        ...
        if args.provider == "OLLAMA1":
            try:
                ...
                validated_predictions.append(state['validation']).split("</think>")[1]
            except:
                predicted_labels.append("NA")
                reasoning_outputs.append("NA")
                validated_predictions.append("NA")
        else:
            predicted_labels.append(state['diagnosis'])
            reasoning_outputs.append(state['reasons']) 
            validated_predictions.append(state['validation']) 
        ...
        print(idx)
        t = idx + 1
        result_bundle = [reasoning_outputs[:t], predicted_labels[:t], actual_labels[:t], validated_predictions[:t]]
        save_results_to_csv(result_bundle, args.results_dir, args.model)

    result_bundle = [reasoning_outputs, predicted_labels, actual_labels, validated_predictions]
    save_results_to_csv(result_bundle, args.results_dir, args.model)
